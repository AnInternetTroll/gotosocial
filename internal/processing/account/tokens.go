...
func (p *Processor) TokensGet(
	...
	for _, token := range tokens {
		...
		if err != nil {
			log.Errorf(ctx, "error converting token to api tokenInfo: %v", err)
			continue
		}

		// Append to return items.
...
func (p *Processor) TokenGet(
	...
	if err != nil && !errors.Is(err, db.ErrNoEntries) {
		err = gtserror.Newf("db error getting token: %w", err)
		return nil, gtserror.NewErrorInternalError(err)
	}

	if token == nil || token.UserID != userID {
		err = gtserror.New("token not found")
		return nil, gtserror.NewErrorNotFound(err)
	}

	apiTokenInfo, err := p.converter.TokenToAPITokenInfo(ctx, token)
	if err != nil {
		err = gtserror.Newf("error converting token to api tokenInfo: %w", err)
		return nil, gtserror.NewErrorInternalError(err)
	}
...
func (p *Processor) TokenInvalidate(
	...
	if err := p.state.DB.DeleteTokenByID(ctx, tokenID); err != nil {
		err = gtserror.Newf("db error deleting token: %w", err)
		return nil, gtserror.NewErrorInternalError(err)
	}
